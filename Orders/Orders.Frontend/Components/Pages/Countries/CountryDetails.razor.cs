...
public partial class CountryDetails
{
    ...
    private async Task ShowModalAsync(int id = 0, bool isEdit = false)
    {
        ...
        if (isEdit)
        {
            ...
            {
                { "Id", id }
            }; dialog = await DialogService.ShowAsync<StateEdit>("Editar Provincia / Estado", parameters, options);
        }
        else
        {
            var parameters = new DialogParameters
            ...
            dialog = await DialogService.ShowAsync<StateCreate>("Nueva Provincia / Estado", parameters, options);
        }
