...
public partial class Register
{
    ...
    {
        var responseHttp = await Repository.GetAsync<List<Country>>("/api/countries/comboCountries");
    ...
    {
        var responseHttp = await Repository.GetAsync<List<State>>($"/api/states/comboStatesByCountry/{countryId}");
        if (responseHttp.Error)
        {
            var message = await responseHttp.GetErrorMessageAsync();
            Snackbar.Add(message!, Severity.Error);
    ...
    {
        var responseHttp = await Repository.GetAsync<List<City>>($"/api/cities/comboCitiesByState/{stateId}");
        if (responseHttp.Error)
        {
            var message = await responseHttp.GetErrorMessageAsync();
            Snackbar.Add(message!, Severity.Error);
    ...
    private async Task CreateUserAsync()
    {
        ...
        await DialogService.ShowMessageBoxAsync(
                "Confirmación",
                "Su cuenta ha sido creada con éxito. Se te ha enviado un correo electrónico con las instrucciones para activar tu usuario.",
                yesText: "Entendido");
        NavigationManager.NavigateTo("/");
    }
}